            ...
            <Pressable className="active:opacity-80">
              ...
              <View className="flex-row items-center gap-3">
                <View className={cn('h-14 w-14 items-center justify-center rounded-2xl', needsTriage ? 'bg-vc-text-dark' : 'bg-vc-primary-dark')}>
                  <Text className={cn('text-base font-bold', needsTriage ? 'text-vc-danger-dark' : 'text-vc-bg-dark')}>{patientInitials(visit.patientId)}</Text>
                </View>
                <View className="flex-1">
                  <Text className={cn('text-base font-bold', needsTriage ? 'text-vc-bg-dark' : 'text-vc-text-dark')}>{needsTriage ? 'Solicitação pendente' : `Paciente ${patientInitials(visit.patientId)}`}</Text>
