            ...
            <View className="gap-3 rounded-2xl border border-vc-danger-dark bg-vc-danger-dark/90 p-4">
              ...
              <View className="flex-row items-center gap-3">
                <View className="h-14 w-14 items-center justify-center rounded-2xl bg-vc-text-dark">
                  <Text className="text-base font-bold text-vc-danger-dark">{((n) => { const p = n.trim().split(/\s+/); return p.length >= 2 ? (p[0][0] + p[p.length-1][0]).toUpperCase() : n.slice(0,2).toUpperCase(); })(patientMap[visit.patientId ?? ''] ?? '?')}</Text>
                </View>
                <View className="flex-1">
                  <Text className="text-lg font-bold text-vc-bg-dark">Atribuir atendimento</Text>
                  <Text className="text-xs text-vc-bg-dark">{patientMap[visit.patientId ?? ''] ?? 'Paciente'} | {visit.reason}</Text>
                </View>
